...
const addTodo = async ({ page, todoObject }: { page: Page; todoObject: TodoElem }) => {
    await page.request.post(endpoint, { data: todoObject });
};

const updateTodo = async ({ page, patchObject }: { page: Page; patchObject: TodoElem }) => {
    await page.request.patch(`${endpoint}/${todoObject.id}`, { data: patchObject });
};
const deleteTodo = async ({ page, ObjectToDelete }: { page: Page; ObjectToDelete: TodoElem }) => {   //defines método AÑADIR elemento definido antes, recuerda estructura - elemento es el param entrada
...
test.describe('POST demo @post2', () => {
    ...
    test('DELETE todo', async ({ page }) => {
        await deleteTodo({page, ObjectToDelete: patchObject});
